...
TABLE_COLUMNS = ['h2h price','spread price','spread value','over price','under price','over/under value']
#GLOBAL VARIABLES END

#caching functions
@st.cache_data
def getLeagueData(bookPass):
    ...
    df = pd.DataFrame(data=TEAMS_PRICE)
    return df
#End of getLeagueData

@st.cache_data
def getSpecTeam(selectedTeam,marketPass,marketSelect):
    ...
    if set(marketSelect) == {"h2h"}:
        ...
        table.columns = ['H2H Price','Spread Price','Spread Line']
        ...
        table.columns = ['Spread Price','Spread Line','Over Price','Under Price','Over/Under Line']
        ...
        table.columns = ['H2H Price','Over Price','Under Price','Over/Under Line']
        ...
        table.columns = ['H2H Price','Spread Price','Spread Line','Over Price','Under Price','Over/Under Line']
    return table
    #End of getSpecTeam
...
def main():
    ...
    if(category == 'Specific NFL Team Data'):
        ...
        if(go and marketSelect and selectedTeam):
            ...
            #calling api which needs selectedTeam, marketPass, marketSelect
            table = getSpecTeam(selectedTeam,marketPass,marketSelect)
            #Formatting the table
            table = table.transpose()
            table.index.name = selectedTeam
            table.columns = RADIO_BOOKS
            #writing the table
            st.dataframe(data=table,use_container_width=True,height=200)
        ...
        elif(go and not selectedTeam):
            st.warning(f'Please be sure to select a team to get results.')
        ...
        st.header("Roadmap of Planned Future Updates",divider="blue")

        
        st.write("1. Update code to store data on a page until a new call is made if possible.")
        st.write("2. Look for a different way to get NFL scores for the Scores Check page.")
        st.write("3. Expand site coverage from NFL to MLB.")
        st.write("4. Add a section that uses a developed model to predict outcomes.")
        ...
        st.write('''The API call currently being used only retrieves NFL scores from at most, 
                 ...
                 if I should use a different API altogether to get these scores. This page is
                 currently disabled to prevent extra API calls from being used while no results
                 can be displayed.''')
        stop = False
